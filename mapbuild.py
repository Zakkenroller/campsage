#!/usr/bin/env python3
"""
mapbuild.py — renders the CampSage interactive map (Leaflet + OpenStreetMap, no API keys) as a
self-contained HTML string from the scan output (status.json + wiki_images.json).

One source of truth, used by BOTH:
  • campsage_web.py   — the /camp/map Flask route (dynamic, local dev)
  • build_site.py     — the static site build (GitHub Actions → Netlify)

No Flask dependency, so the static build stays dependency-free.
"""
import json
...
def render_map_html(data_dir, list_href="/camp"):
    """Return the full self-contained map page as a string.

    data_dir : folder holding status.json + wiki_images.json (config.DATA_DIR).
    list_href: where the '← List' link points — '/camp' for the Flask route,
               'index.html' for the static Netlify build.
    """
    data_dir = Path(data_dir)
    status = load_json_safe(data_dir / "status.json", {})
    wiki = load_json_safe(data_dir / "wiki_images.json", {})
    pts = _points(status, wiki)
    home = status.get("home") or "home"
    return (_CAMP_MAP_HTML
            .replace("__DATA__", json.dumps(pts))
            .replace("__HOME__", str(home))
            .replace("__N__", str(len(pts)))
            .replace("__LIST_HREF__", list_href)
            .replace("__HOMELAT__", str(config.HOME_LAT))
            .replace("__HOMELNG__", str(config.HOME_LNG)))
...
<div class="bar"><a href="__LIST_HREF__">← List</a><b>🏕️ CampSage Map</b><span class="muted" id="count">__N__ shown</span></div>
...
<script>
const PTS = __DATA__;
const map = L.map('map',{zoomControl:true}).setView([__HOMELAT__,__HOMELNG__], 8);
